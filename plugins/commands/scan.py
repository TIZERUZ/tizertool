...
def mcscan2(server, ports, online_filter, maximum_ping ,save=False, file=None, version_filter=None):
    if not checkserver(server): logging.error('Iltimos, Real domen kiriting:'); return

    if '-' in ports: start, end = map(int, ports.split('-'))
    else: start = end = int(ports)

    servers = wow(server)
    if start < 1 or end > 65535: logging.error('Iltimos haqiqiy qiymat bering'); return
...
def mcscan(server, ports, mthreads):
    ...
    servers = wow(server)
    if start < 1 or end > 65535: logging.error('Iltimos, Real Qiymat kiriting'); return
    if start > end: logging.error('Start port has to be <= end port'); return
    threads = []
...
def tcpping(server, port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if s.connect_ex((server, port)) == 0:
                logging.success(f"{red}({white}{server}{red})({white}{port}{red})({white}{service(port)}{red})")

def scan(server, ports, mthreads):
    if not checkip(server): logging.error('Iltimos, Real domen kiriting:'); return
    servers = wow(server)
    if '-' in ports: start, end = map(int, ports.split('-'))
    else: start = end = int(ports)
    if start < 1 or end > 65535: logging.error('Real qiymat bering'); return
    if start > end: logging.error('Start port has to be <= end port'); return
    monkey = []
    for host in servers:
        for port in range(start, end + 1):
